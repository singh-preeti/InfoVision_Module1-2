package com.day1;

class Employee{
	// global variable
	  // int emp_id =102;
	  // String emp_name = "Pawan";
	 // static  String company_name = "InfoVision";

	   public double getSalary(double salary,double bonus){
		   int year_of_exp = 6;
		   float performance_per = 60;
	     //  for(int i = 0; i<100; i++){   itration is 100 times  i =2........ i =100}
	       double total_salary = salary + bonus;
           double full_salary = total_salary + 1000;
	       
//           if(year_of_exp>=5 || performance_per >=70) {
//	    	   System.out.println("bonus" +full_salary);
//	       }
//	       else
//	       {
//	    	   System.out.println("total_salary" +salary);
//	       }
           
           if(year_of_exp == 5)
           {
        	 System.out.println(total_salary);  
           }
           else if(year_of_exp > 5 && year_of_exp <= 8){
        	  System.out.println(full_salary); 
           }
           else if(year_of_exp >=5 && performance_per == 60) {
        	   System.out.println(total_salary);
           }
           else {
        	   System.out.println(salary);
           }
		return total_salary;
	       
	       
	   }
	   private void elseif(boolean b) {
		// TODO Auto-generated method stub
		
	}
	public  static void display_company_name(){
	       String comp_name = "InfoVision";
	      System.out.println("the name of company of all the employee" +comp_name);
	   }
	 
	 public static void main(String args[]){
	      Employee.display_company_name();
	     Employee emp = new Employee();
	     emp.getSalary(20000,2000);
	  }
	}





